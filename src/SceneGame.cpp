...
SceneGame::SceneGame(WorkingDirectory& workingDir, ResourceAllocator<sf::Texture>& textureAllocator, Window& window) : workingDir(workingDir), textureAllocator(textureAllocator), mapParser(textureAllocator, context), objects(drawableSystem, collisionSystem), window(window) { }
...
void SceneGame::OnCreate()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		idleAnimation->AddFrame(playerTextureID, 0, idleYFramePos,
			frameWidth, frameHeight, 0.f, idleAnimationLooped);
	...
	for (int i = 0; i < 4; i++)
	{
		...
		for (int i = 0; i < walkingFrameCount; i++)
		{
			walkingAnimation->AddFrame(playerTextureID, i * frameWidth,
				...
				frameHeight,
				delayBetweenWalkingFramesSecs, walkAnimationLooped);
		}
	...
	*************************/
	...
	for (int i = 0; i < 4; i++)
	{
		...
		for (int i = 0; i < projectileFrameCount; i++)
		{
			projAnimation->AddFrame(playerTextureID, i * frameWidth,
				...
				delayBetweenProjectileFramesSecs, projectileAnimationLooped);
		}
	...
	std::vector<std::shared_ptr<Object>> levelTiles =
		mapParser.Parse(workingDir.Get() + "Path.tmx",
			mapOffset);
